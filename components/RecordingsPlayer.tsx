...
const rangeThumbStyles = `
  ...
  [data-seek]::-webkit-slider-thumb:active {
    transform: scale(1.3);
  }
  ...
  [data-seek]::-webkit-slider-runnable-track {
    ...
    background: linear-gradient(to right, #3b82f6 var(--pct), #374151 var(--pct));
  }
  [data-seek]::-moz-range-track {
    height: 4px;
    border-radius: 9999px;
    background: linear-gradient(to right, #3b82f6 var(--pct), #374151 var(--pct));
  }
...
export default function RecordingsPlayer({ src }: { src: string }) {
  ...
  useEffect(() => {
    ...
    const onTime = () => { if (!seekingRef.current) setCurrent(el.currentTime) }
