...
async function fetchIntroMeetings(): Promise<Company[]> {
  ...
  // Within each group, sort by next_reach_out asc
  return rows
}
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-0.5">
                {all.length} compan{all.length === 1 ? 'y' : 'ies'} want an intro meeting
              </p>
            </div>
          ...
          {high.length > 0 && (
            <section className="space-y-3">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-gray-600 font-medium">{high.length}</span>
              </div>
          ...
          {other.length > 0 && (
            <section className="space-y-3">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-gray-600 font-medium">{other.length}</span>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {other.map(c => (
                  <MeetingCardClient key={c.id} company={c} />
